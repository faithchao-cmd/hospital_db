...
WHERE appointment_date > '2025-01-01';
  ...
  select ,patients_id ,f_name, l_name, count (DOB) as Age
  from Patients
  where Age > 50
  group by patient_id; 
...
CREATE TABLE billing (
    ...
    payment_status  VARCHAR(20),  -- Paid, Partial, Pending, Insurance Pending
...
WHERE  EXISTS (
  SELECT 1
  FROM   Rooms r
  WHERE  r.department_id = dep.department_id   -- room belongs to THIS dept
    AND  r.status = 'Available'                -- and is available
)
AND    EXISTS (
  SELECT 1
  FROM   Doctors d
  WHERE  d.department_id = dep.department_id   -- doctor belongs to THIS dept
)
ORDER BY dep_name;
